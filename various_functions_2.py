...
def bono_tasa_fija(start_time, num_cupones, valor_tasa):
    tiempos = []
    flujos = []
    yf = .5
    for i in range(0, num_cupones):
        tiempos.append(i * yf + start_time)
        flujo = valor_tasa * yf
        if i == num_cupones - 1:
            flujo += 1.0
        flujos.append(flujo)
    return tiempos, flujos
...
def intereses_pata_flotante(fecha_hoy, fechas, nocional, curva_interpol, tipo_tasa='LinAct/360'):
    ...
    for x in zip(nocional, fechas):
        ...
        valor_tasa = (df1 / df2 - 1.0) * 360.0 / (dias2 - dias1)
        dias = int((x[1][1] - x[1][0]).days)
        interes = x[0] * valor_tasa * dias / 360.0
        resultado.append( (x[0], x[1][0], x[1][1], interes) )
    return resultado
...
def valor_swap(tasa_fija, fecha_hoy, fechas, nocional, intereses_pata_flotante, curva_interpol):
    int_pata_fija = intereses_pata_fija(fechas, nocional, tasa_fija)
    vp_fija = valor_presente(fecha_hoy, int_pata_fija, curva_interpol)
    vp_flotante = valor_presente(fecha_hoy, intereses_pata_flotante, curva_interpol)
    return vp_fija - vp_flotante
